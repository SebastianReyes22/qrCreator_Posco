<?php
    if(!isset($_FILES['file-1']['tmp_name'])){
        header("location:../../pages/error.html");
    }
    
    function getSerial(){
        require "connection.php";
        $query = "SELECT serial FROM budomari WHERE serial=634 LIMIT 1";
        $result = mysqli_query($connection,$query);
        mysqli_close($connection);
        if($result = mysqli_fetch_array($result)){
            if((empty($result['serial']) && $result['serial']!=0) || !is_numeric($result['serial']) || $result['serial']<0){
